...
1.  **Add the GPG Key:**
    ```bash
    curl -s "https://jeka.github.io/naive_backup/public.key" | gpg --dearmor | sudo tee /usr/share/keyrings/naive-backup-archive-keyring.gpg >/dev/null
    ```
...
1.  **Add the GPG Key:**
    ```bash
    curl -s "https://jeka.github.io/naive_backup/public.key" > /tmp/public.key
...
2.  **Add the Repository:** Add the following line to `/etc/opkg/customfeeds.conf`:
    ```
    src/gz custom_naive_backup https://jeka.github.io/naive_backup/opkg
    ```
